...
export async function GET(RequestEvent){
    ...
    const { recipeIndex } = params; // Should be the name of the route
...
export async function PATCH(RequestEvent){
    ...
    if(new_name == "" || recipes_names.includes(new_name)){ 
        ...
        db.collection("recipes").updateOne({user:user_email, name:recipes_names[recipeIndex] }, {$set : {name:new_name}});
    }

    return json(
        {
            _id: recipes_data[recipeIndex]._id, 
            user:user_email, name: new_name, 
...
export async function DELETE(RequestEvent){
    ...
    const { params } = RequestEvent;
    const { recipeIndex } = params;

    const deleted = {user: user_email, name:recipes_names[recipeIndex]}
